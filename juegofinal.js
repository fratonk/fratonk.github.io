...
class Juego {
    ...
    toggleBtnEmpezar() {
        if (btnEmpezar.classList.contains('hide') && overlay.classList.contains('hide')) {
    ...
    ganoElJuego () {
        swal ('Simon Dice','Felicitaciones, Alvis ganaste el juego, te quiero Muchoooo', 'success')
        .then(() => {
            this.inicializar()
        })
    }
    ...
    perdioElJuego () {
        swal ('Simon Dice','Lo lamentamos, perdiste Alvis, llega hasta el final', 'error')
        .then(() => {
            this.eliminarEventosClick()
            this.inicializar()
        })
